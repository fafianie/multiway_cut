...
uint64_t Galois::clmul_multiply(uint64_t arg1, uint64_t arg2)
{
	...
	/*__asm{ 	//carry-less multiplication
		"movd %3, %%xmm1;"
		"movd %4, %%xmm2;"
...
uint64_t Galois::uniform_random_element()
{
	...
	uniform_int_distribution<> dist16(0, static_cast<int>(pwm1[16]));
	...
	uniform_int_distribution<> distb(0, static_cast<int>(pwm1[bits]));
	rand <<= bits;
	rand ^= distb(gen);
